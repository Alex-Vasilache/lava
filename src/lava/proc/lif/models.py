...
from lava.proc.lif.process import (LIF, FlexLIF, LIFReset, TernaryLIF, LearningLIF,
                                   LIFRefractory)
...
class AbstractPyFlexLifModelFloat(PyLoihiProcessModel):
    """Abstract implementation of floating point precision
    leaky-integrate-and-fire neuron model with individual thresholds.
...
class AbstractPyFlexLifModelFixed(PyLoihiProcessModel):
    """Abstract implementation of fixed point precision
    leaky-integrate-and-fire neuron model with individual thresholds.
    
    Implementations like those
    ...
    def __init__(self, proc_params):
        super(AbstractPyFlexLifModelFixed, self).__init__(proc_params)
...
@implements(proc=FlexLIF, protocol=LoihiProtocol)
@requires(CPU)
@tag("floating_pt")
class PyFlexLifModelFloat(AbstractPyFlexLifModelFloat):
    """Implementation of Leaky-Integrate-and-Fire with individual thresholds 
    neural process in floating point precision. This short and simple ProcessModel 
    can be used for quick algorithmic prototyping, without engaging with the 
...
class PyFlexLifModelBitAcc(AbstractPyFlexLifModelFixed):
    """Implementation of Leaky-Integrate-and-Fire with individual thresholds 
    neural process bit-accurate with Loihi's hardware LIF dynamics, which means, 
    ...
    def __init__(self, proc_params):
        super(PyFlexLifModelBitAcc, self).__init__(proc_params)
        self.effective_vth = np.zeros_like(self.vth)
